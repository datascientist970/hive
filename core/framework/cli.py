...
def cmd_validate(args: argparse.Namespace) -> int:
    ...
    # 1. Graph Structure Validation (NEW)
    print("\n" + "=" * 60)
    print("GRAPH STRUCTURE VALIDATION")
    print("=" * 60)
    
    graph_validator = GraphDependencyValidator(runner.graph)
    graph_validation = graph_validator.validate()
    ...
    if not graph_validation.valid:
        print("\n Agent validation failed due to graph structure errors.")
        runner.cleanup()
        return 1

    print("\n Graph structure is valid")
    print(f"   Nodes: {len(runner.graph.nodes)}")
    print(f"   Edges: {len(runner.graph.edges)}")
    print(f"   Entry node: {runner.graph.entry_node}")

    # 2. Tools and Credentials Validation (existing)
    validation = runner.validate()

    print("\n" + "=" * 60)
    print("TOOLS & CREDENTIALS VALIDATION")
    print("=" * 60)
    ...
    # Return 0 only if both validations pass
    return 0 if (graph_validation.valid and validation.valid) else 1
